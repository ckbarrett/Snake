...
function checkCollisions(sn) {
	...
	// Define collisions
	let hitRightWall =
		sn.head.x + squareWidth + xtile > width &&
		sn.xDir[sn.xDir.length - 1] == 1;
	let hitLeftWall =
		sn.head.x - xtile < 0 && sn.xDir[sn.xDir.length - 1] == -1;
	let hitBottomWall =
		sn.head.y + squareWidth + ytile > height &&
...
function determineAmpleRemainingSpace() {
	...
	while (q.tail - q.head != 0) {
		...
		let leftSquare = new Square(
			current.x - oneHorizontalTile,
			current.y,
			squareWidth
		);

		q.enqueue(leftSquare);
		let upSquare = new Square(
			current.x,
			current.y - oneVerticalTile,
			squareWidth
		);
		q.enqueue(upSquare);
...
function mouseReleased() {
	if (gameOver) {
		...
		if (
			mouseX >= playAgainx &&
			mouseX <= playAgainx + playAgainRectWidth &&
			mouseY >= playAgainy &&
			mouseY <= playAgainy + playAgainRectHeight
		) {
			restartGame();
		}
	}
